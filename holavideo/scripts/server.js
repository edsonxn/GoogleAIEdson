...
const MAX_SLOTS = parseInt(process.env.HV_SLOTS || "3");
const _freeSlots = new Set(Array.from({ length: MAX_SLOTS }, (_, i) => i));
const _slotWaiters = [];
function acquireSlot() {
    const iter = _freeSlots.values();
    const first = iter.next();
    if (!first.done) {
        _freeSlots.delete(first.value);
...
function buildDimNote(width, height) {
...
async function tryWithKey(apiKey, model, prompt, durationSeconds, timeoutMs = 30000, width = 1280, height = 720, systemPrompt = SYSTEM_PROMPT) {
...
async function tryWithOllama(prompt, durationSeconds, width = 1280, height = 720, ollamaModel = "gemma4") {
...
function fillTemplate(templateId, variables, durationSeconds, width, height) {
    const template = TEMPLATES.find(t => t.id === templateId);
    if (!template) {
        console.log(`  ⚠️ Template no encontrado: "${templateId}"`);
        return null;
    }
...
const TEMPLATE_LIST_FOR_PROMPT = TEMPLATES.map((t, i) => `${i + 1}. id="${t.id}" — ${t.name}\n   Usar cuando: ${t.description}\n   Variables: ${t.vars.map(v => `${v.name} (${v.hint})`).join(' | ')}`).join('\n\n');
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function pickAndFillTemplate(prompt, durationSeconds, width, height) {
    ...
    for (const key of FREE_KEYS) {
        try {
            ...
            if (!parsed.templateId || !parsed.variables)
                continue;
...
async function generateVideo(prompt, durationSeconds, styleContext, preferredModel, outputFilename, width = 1280, height = 720, allowImages = false, injectedCode, slotId = 0, modelChain) {
    ...
    if (injectedCode) {
    ...
    else if (preferredModel === "ollama" || (preferredModel && preferredModel.startsWith("ollama:")) ||
        ...
        const resolveModel = (v) => ALIAS_MAP[v] ?? v;
        let attempts;
    ...
    if (remotionImportLine) {
        const imported = remotionImportLine[1].split(',').map((s) => s.trim());
        ...
        for (const [sym] of Object.entries(remotionSymbols)) {
            ...
            if (usedInCode && !imported.includes(sym))
                toAdd.push(sym);
        }
        if (toAdd.length > 0) {
    ...
    const assetCandidates = publicFiles.filter(f => f.startsWith('hv_web_') || f.startsWith('person_') || f.startsWith('entity_') || f.startsWith('ra_manual_'));
    const charOverlapScore = (a, b) => {
    ...
    const renderOnce = () => (0, child_process_1.execSync)(`npx remotion render src/index_s${slotId}.ts MyComp out/${outFile} --overwrite --timeout=90000`, { cwd: PROJECT_ROOT, stdio: ["ignore", "pipe", "pipe"], timeout: 300000, maxBuffer: 50 * 1024 * 1024 });
    const isCodeError = (msg) => /ReferenceError|SyntaxError|TypeError|is not defined|Cannot read prop|Unexpected token|is not a function|can not be undefined|inputRange can not|outputRange can not|is not iterable|Cannot destructure|passed to the `component` prop|value of `undefined` was passed|Transform failed|Unexpected "="/i.test(msg);
    const stripAnsi = (s) => s.replace(/\x1B\[[0-9;]*[mGKHFABCDJST]/g, '');
    try {
        renderOnce();
...
app.use("/out", express_1.default.static(path.join(PROJECT_ROOT, "out")));
app.use("/public", express_1.default.static(path.join(PROJECT_ROOT, "public")));
app.get("/", (_req, res) => {
...
app.post("/api/generate", async (req, res) => {
    ...
    let imageNote = '';
    let hasVideo = assetList.some((a) => a.use === 'video' || a.filename?.match(/\.(mp4|webm|mov)$/i));
    if (assetList.length) {
        console.log(`🖼️ Con ${assetList.length} asset(s): ${assetList.map((a) => a.filename).join(', ')}`);
        const positionCSS = {
        ...
        const descriptions = assetList.map((a, i) => {
  ...
  ${isVideo ? '' : `Animación: ${anim}\n  `}${overlayNote}`;
        ...
        const elementAssets = assetList.filter((a) => a.use === 'element');
        const hasRealImage = elementAssets.length > 0;
        let overrideNote = '';
        if (elementAssets.length >= 2) {
            const names = elementAssets.map((a) => `"${a.filename}"`).join(' y ');
